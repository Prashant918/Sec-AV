...
class AntivirusGUI:
    ...
    def _create_menu(self):
        ...
        if self.threat_engine:
            ...
            file_menu.add_command(
                label="Scan Folder... (Unavailable)", state=tk.DISABLED
            )
        ...
        if self.signature_manager:
            tools_menu.add_command(
                label="Update Signatures", command=self._update_signatures
            )
        else:
            tools_menu.add_command(
                label="Update Signatures (Unavailable)", state=tk.DISABLED
            )

        if self.quarantine_manager:
            tools_menu.add_command(
                label="Quarantine Manager", command=self._open_quarantine_manager
            )
        else:
            tools_menu.add_command(
                label="Quarantine Manager (Unavailable)", state=tk.DISABLED
            )

        if self.realtime_monitor:
            tools_menu.add_command(
                label="Real-time Protection", command=self._toggle_realtime_protection
            )
        else:
            tools_menu.add_command(
                label="Real-time Protection (Unavailable)", state=tk.DISABLED
            )
    ...
    def _create_scan_tab(self):
        ...
        ttk.Radiobutton(
            options_frame, text="Quick Scan", variable=self.scan_type, value="quick"
        ).grid(row=0, column=1, sticky=tk.W, padx=5)
        ttk.Radiobutton(
            options_frame, text="Full Scan", variable=self.scan_type, value="full"
        ).grid(row=0, column=2, sticky=tk.W, padx=5)
        ttk.Radiobutton(
            options_frame, text="Custom Scan", variable=self.scan_type, value="custom"
        ).grid(row=0, column=3, sticky=tk.W, padx=5)

        # Scan path
        ttk.Label(options_frame, text="Scan Path:").grid(
        ...
        ttk.Button(options_frame, text="Browse", command=self._browse_scan_path).grid(
        ...
        self.stop_button = ttk.Button(
            button_frame, text="Stop Scan", command=self._stop_scan, state=tk.DISABLED
        )
    ...
    def _create_protection_tab(self):
        ...
        self.protection_status = ttk.Label(
            status_frame,
            text="Real-time Protection: Disabled",
            font=("Arial", 12, "bold"),
        )
        self.protection_status.pack(pady=10)

        self.protection_button = ttk.Button(
            status_frame, text="Enable Protection", command=self._toggle_protection
        )
        ...
        ttk.Checkbutton(
            settings_frame, text="Monitor Downloads", variable=self.monitor_downloads
        ).pack(anchor=tk.W, padx=10, pady=2)

        self.monitor_usb = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            settings_frame, text="Monitor USB Devices", variable=self.monitor_usb
        ).pack(anchor=tk.W, padx=10, pady=2)
        ...
        self.threats_tree = ttk.Treeview(
            threats_frame, columns=threat_columns, show="headings", height=10
        )
        ...
        threats_scrollbar = ttk.Scrollbar(
    ...
    def _create_quarantine_tab(self):
        ...
        ttk.Button(
            controls_frame, text="Refresh", command=self._refresh_quarantine
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            controls_frame, text="Restore Selected", command=self._restore_selected
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            controls_frame, text="Delete Selected", command=self._delete_selected
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            controls_frame, text="Clean Old Items", command=self._clean_quarantine
        ).pack(side=tk.LEFT, padx=5)

        # Quarantine list
        quarantine_columns = ("ID", "Original Path", "Threat", "Date", "Status")
        self.quarantine_tree = ttk.Treeview(
            quarantine_frame, columns=quarantine_columns, show="headings", height=20
        )
        ...
        quarantine_scrollbar = ttk.Scrollbar(
            quarantine_frame, orient=tk.VERTICAL, command=self.quarantine_tree.yview
        )
        self.quarantine_tree.configure(yscrollcommand=quarantine_scrollbar.set)

        self.quarantine_tree.pack(
    ...
    def _create_statistics_tab(self):
        ...
        # Refresh button
        ttk.Button(
            stats_frame, text="Refresh Statistics", command=self._refresh_statistics
        ).pack(pady=5)
    ...
    def _create_logs_tab(self):
        ...
        ttk.Button(log_controls, text="Clear", command=self._clear_logs).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def _create_status_bar(self):
        ...
        # Signature status
        if self.signature_manager:
            self.signature_status = ttk.Label(
                self.status_bar, text="Signatures: Loading..."
            )
        else:
            self.signature_status = ttk.Label(
                self.status_bar, text="Signatures: Unavailable"
            )
            ...
            try:
                ...
                if self.signature_manager:
                    try:
                        ...
                        total_sigs = stats.get("hash_signatures", 0) + stats.get(
                ...
                if self.realtime_monitor:
                    if self.monitoring:
                        self.protection_status.config(
                            text="Real-time Protection: Enabled", foreground="green"
                        )
                        self.protection_button.config(text="Disable Protection")
                    else:
                        self.protection_status.config(
                            text="Real-time Protection: Disabled", foreground="red"
                        )
        ...
        try:
            ...
            for file_path in files_to_scan:
                ...
                try:
                    # Update progress
                    progress = (
                        (scanned_files / total_files) * 100 if total_files > 0 else 0
                    )
                    self.root.after(
                        0,
                    ...
                    self.root.after(
                        0, self._add_scan_result, file_path, status, threat, action
                    )
                    ...
                    self.root.after(
                        0, self._add_scan_result, file_path, "ERROR", str(e), "None"
                    )
    ...
    def _add_scan_result(self, file_path, status, threat, action):
        ...
        self.results_tree.insert(
            "",
            tk.END,
            values=(os.path.basename(file_path), status, threat, action),
            tags=tags,
        )
    ...
    def _update_signatures(self):
        ...
        def update_worker():
            try:
                self.root.after(
                    0, lambda: self.status_label.config(text="Updating signatures...")
                )
                ...
                if success:
                    self.root.after(
                        ...
                        lambda: messagebox.showinfo(
                            "Update", "Signatures updated successfully"
                        ),
                    )
                else:
                    self.root.after(
                ...
                self.root.after(
                    0, lambda: messagebox.showerror("Error", f"Update failed: {e}")
                )
                self.root.after(0, lambda: self.status_label.config(text="Ready"))
    ...
    def _clean_quarantine(self):
        ...
        if messagebox.askyesno(
            "Confirm", "Clean old quarantine items (older than 30 days)?"
        ):
            try:
    ...
    def _refresh_statistics(self):
        ...
        if not self.signature_manager:
            ...
            self.stats_text.insert(
                1.0, "Statistics not available - signature manager not loaded"
            )
            return
