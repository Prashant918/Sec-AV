...
class FileScanner:
    ...
    def scan_directory(
        self, directory_path: str, recursive: bool = True, quarantine: bool = False
        ...
        try:
            ...
            for item in file_iterator:
                if item.is_file():
                    ...
                    if any(
                        excluded in str(item.parent) for excluded in self.excluded_dirs
                    ):
                        continue
            ...
            yield {"status": "error", "message": f"Error scanning directory: {e}"}
    ...
    def quick_scan(self, quarantine: bool = False) -> Dict:
        ...
        for location in scan_locations:
            if location and os.path.exists(location):
                ...
                for result in self.scan_directory(
                    location, recursive=False, quarantine=quarantine
                ):
    ...
    def full_scan(self, quarantine: bool = False) -> Dict:
        ...
        for result in self.scan_directory(
            scan_root, recursive=True, quarantine=quarantine
        ):
            if result["status"] == "infected":
                results["threats_found"] += 1
                results["infected_files"].append(result)
                print(
                    colored(
                        f"THREAT FOUND: {result.get('threat_name')} in {result.get('file_path')}",
                        "red",
                    )
                )
            elif result["status"] == "suspicious":
            ...
            if result["status"] in ["clean", "infected", "suspicious"]:
                ...
                if results["files_scanned"] % 100 == 0:
                    print(
                        colored(
                            f"Progress: {results['files_scanned']} files scanned, {results['threats_found']} threats found",
                            "blue",
                        )
                    )
...
"""
        ...
        if scan_results["infected_files"]:
            ...
            for infected in scan_results["infected_files"]:
                ...
                report += (
                    f"    Method: {infected.get('detection_method', 'Unknown')}\n\n"
                )
