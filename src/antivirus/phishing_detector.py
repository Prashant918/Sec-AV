...
class PhishingDetector:
    ...
    def _analyze_urls(self, urls: List[str]) -> Dict:
        ...
        # Normalize score
        if urls:
            analysis["malicious_score"] = min(
                analysis["malicious_score"] / len(urls), 1.0
            )
            ...
            for legit_domain in legitimate_domains:
                if self._is_typosquatting(domain, legit_domain):
                    ...
                    result["indicators"].append(
                        f"Possible typosquatting of {legit_domain}"
                    )
    ...
    def _is_typosquatting(self, domain: str, legitimate_domain: str) -> bool:
        ...
        return (
            1 <= distance <= 3
            and abs(len(domain) - len(legitimate_domain)) <= 2
            and domain != legitimate_domain
        )
    ...
    def _analyze_phone_number(self, phone_number: str) -> Dict:
        ...
        if cached_result and cached_result["is_suspicious"]:
            result["score"] += 0.6
            result["indicators"].append(
                f"Known suspicious number (reported {cached_result['report_count']} times)"
            )
    ...
    def _analyze_call_content(self, content: str) -> Dict:
        ...
        result = {"score": 0.0, "indicators": [], "call_type": "unknown"}
    ...
    def analyze_message(
        self, message_content: str, sender: str = "", platform: str = ""
    ) -> Dict:
        ...
        for pattern in financial_patterns:
            if pattern in content_lower:
                ...
                analysis_result["indicators"].append(
                    f"Financial scam indicator: {pattern}"
                )
    ...
    def report_phone_number(self, phone_number: str, call_type: str):
        ...
        try:
            ...
            print(
                colored(
                    f"Phone number {phone_number} reported as {call_type}", "yellow"
                )
            )
