...
class NetworkSecurityScanner:
    ...
    def scan_network_connections(self) -> Dict:
        ...
        try:
            ...
            for conn in connections:
                ...
                status = conn.status
                scan_result['connection_summary'][status] = scan_result['connection_summary'].get(status, 0) + 1
            
            print(colored(f"Network scan completed: {len(scan_result['malicious_connections'])} malicious, "
                         f"{len(scan_result['suspicious_connections'])} suspicious connections", 'green'))
    ...
    def _analyze_connection(self, connection) -> Dict:
        ...
        analysis = {
            ...
            'remote_address': f"{connection.raddr.ip}:{connection.raddr.port}" if connection.raddr else "Unknown",
        ...
        if connection.raddr:
            ...
            if port_category:
                ...
                analysis['threat_indicators'].append(f'Suspicious port: {port_category}')
            ...
            if self._is_suspicious_geography(remote_ip):
                analysis['is_suspicious'] = True
                analysis['threat_indicators'].append('Connection to suspicious geographic location')
    ...
    def scan_open_ports(self, target_ip: str = None, port_range: tuple = (1, 1024)) -> Dict:
        """Scan for open ports on target IP"""
        if target_ip is None:
            target_ip = '127.0.0.1'  # Scan localhost by default
        
        print(colored(f"🔍 Scanning ports {port_range[0]}-{port_range[1]} on {target_ip}...", 'blue'))
        ...
        print(colored(f"Port scan completed: {len(scan_result['open_ports'])} open ports found", 'green'))
        
        if scan_result['suspicious_ports']:
            print(colored(f"⚠️ {len(scan_result['suspicious_ports'])} suspicious ports detected!", 'yellow'))
    ...
    def monitor_network_traffic(self, duration: int = 60) -> Dict:
        ...
        print(colored(f"📊 Monitoring network traffic for {duration} seconds...", 'blue'))
        ...
        if bytes_sent > 100 * 1024 * 1024:  # > 100MB sent
            monitoring_result['suspicious_activities'].append('High outbound data volume')
        
        if packets_sent > 10000:  # > 10k packets
            monitoring_result['suspicious_activities'].append('High packet transmission rate')
        
        monitoring_result['end_time'] = end_time
        
        print(colored(f"Traffic monitoring completed: {bytes_sent + bytes_recv} bytes transferred", 'green'))
